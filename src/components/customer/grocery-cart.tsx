...
import { ShoppingCart, X, Plus, Minus, Carrot } from 'lucide-react';
...
export function GroceryCart({ store, isSheet = false }: GroceryCartProps) {
  const { cart, store: cartStore, removeItem, updateItemQuantity, clearCart } = useGroceryCart();
  const { user } = useAuth();
  ...
  if (cart.length > 0 && cartStore?.id !== store.id) {
     return (
        <div className={cartContainerClass}>
            <div className="p-6">
                <div className="flex items-center justify-center gap-3 mb-6">
                    <Carrot className="h-7 w-7"/>
                    <h2 className="text-2xl font-bold">Your Grocery Cart</h2>
                </div>
                <div className="text-center text-muted-foreground py-12">
                    <p>Your cart has items from another store.</p>
