        ...
        const fetchStoreAndOrders = async () => {
            if (user?.uid) {
                try {
                    ...
                    setStore(storeData);
                    if (storeData) {
                        const unsub = listenToOrdersForStore(storeData.id, (allOrders) => {
                            const historicalOrders = allOrders.filter(o => o.status === 'delivered' || o.status === 'cancelled');
                            setOrders(historicalOrders);
                        }, (err) => {
                            setError('Failed to listen for orders.');
                            console.error(err);
                        });
                        // Note: In a real app, you'd want to manage this unsubscribe call properly.
                        // For this component's lifecycle, it's okay as it's a one-time fetch logic for now.
                    } else {
             ...
             <div className="min-h-screen bg-background">
                ...
                <main className="container py-8 flex items-center justify-center">
                    <Alert>
                        ...
                        <AlertDescription>You must register a store before you can view order history.</AlertDescription>
                    </Alert>
                </main>
            </div>
        )
            ...
            <main className="container py-8">
                <div className="flex justify-between items-center mb-8">
                    <h1 className="text-3xl font-bold">Order History</h1>
                     <Button asChild variant="outline">
                        <Link href="/grocery-owner/orders">
                            View Active Orders
                        </Link>
                    </Button>
                </div>
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>A historical record of all orders from {store.name}.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        {orders.length > 0 ? (
