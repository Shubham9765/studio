...
import { addDeliveryBoyToGroceryStore, removeDeliveryBoyFromGroceryStore } from '@/services/ownerService';
import { getGroceryStoreByOwnerId } from '@/services/ownerClientService';
import type { GroceryStore, DeliveryBoy } from '@/lib/types';
...
export default function ManageDeliveryPage() {
    ...
    const fetchStore = async () => {
        if (user?.uid) {
            try {
                const rest = await getGroceryStoreByOwnerId(user.uid);
                setStore(rest);
            } catch (e: any) {
    ...
    const handleAddDeliveryBoy = async (e: React.FormEvent) => {
        ...
        try {
            await addDeliveryBoyToGroceryStore(store.id, email);
    ...
    const handleRemoveDeliveryBoy = async (deliveryBoy: DeliveryBoy) => {
        ...
        try {
            await removeDeliveryBoyFromGroceryStore(store.id, deliveryBoy);
             ...
             <div className="min-h-screen bg-background">
                ...
                <main className="container py-8 flex items-center justify-center">
                    <Alert>
                        ...
                        <AlertDescription>You must register a store before you can manage delivery staff.</AlertDescription>
                    </Alert>
                </main>
            </div>
        )
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle>Your Delivery Team</CardTitle>
                             <CardDescription>
                                {store.deliveryBoys?.length || 0} people on your team.
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
                             {store.deliveryBoys && store.deliveryBoys.length > 0 ? (
